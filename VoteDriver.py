...
x_val = np.reshape(x_test, (10000, 28*28))[:VAL_LEN]
x_test = np.reshape(x_test, (10000, 28*28))[VAL_LEN:VAL_LEN+TEST_LEN]

y_train = y_train[:TRAIN_LEN]
y_val = y_test[:VAL_LEN]
y_test = y_test[VAL_LEN:VAL_LEN+TEST_LEN]
...
#%%
mld = MLD.ML_Democracy()
mld.set_num_classifications(10)
mld.set_default_data(x_train, x_test, y_train, y_test)
mld.add_algo(MLD.ML_Algo(log_init, predict, "logistic reg"), num=5) # can add a cutom dataset
#MLD.add_algo(MLD.ML_Algo(mlp_init, predict, "mlp classifier"))
mld.add_algo(MLD.ML_Algo(lda_init, predict, "lda"), num=3)
mld.add_algo(MLD.ML_Algo(nb_init, predict,  "Nbayes"), num=3)
...
mld.train_algos(bag=True, featureProportion=0.33,x_train=x_train,y_train=y_train,x_val=x_val,y_val=y_val) # add nullable args for funnel or not
mld.current_algos()
